...
type AppointmentHandler struct{ apptSvc service.AppointmentService }
...
func (h *AppointmentHandler) Book(c *gin.Context) {
	...
	if err != nil {
		slog.Warn("book appointment failed", "patient_id", patientID, "slot_id", req.SlotID, "error", err)
...
func (h *AppointmentHandler) GetMyAppointments(c *gin.Context) {
	...
	case models.RoleDoctor:
		appts, err = h.apptSvc.GetDoctorAppointments(callerID)
...
func (h *AppointmentHandler) GetByID(c *gin.Context) {
	...
	callerID := middleware.GetCallerID(c)
	callerRole := middleware.GetCallerRole(c)
	canView := callerRole == models.RoleAdmin ||
		appt.PatientID == callerID ||
		(callerRole == models.RoleDoctor && appt.DoctorID == callerID)
	if !canView {
		utils.ErrorResponse(c, models.ErrForbidden)
		return
	}
...
func (h *AppointmentHandler) Confirm(c *gin.Context) {
	...
	if err := h.apptSvc.Confirm(doctorID, id); err != nil {
		slog.Warn("confirm appointment failed", "appointment_id", id, "doctor_id", doctorID, "error", err)
		utils.ErrorResponse(c, err)
		return
	}
	slog.Info("appointment confirmed", "appointment_id", id, "doctor_id", doctorID)
	utils.SuccessResponse(c, http.StatusOK, gin.H{"message": "appointment confirmed"})
}
...
func (h *AppointmentHandler) Cancel(c *gin.Context) {
	...
	if err := h.apptSvc.Cancel(callerID, callerRole, id); err != nil {
		slog.Warn("cancel appointment failed", "appointment_id", id, "caller_id", callerID, "error", err)
		utils.ErrorResponse(c, err)
		return
	}
